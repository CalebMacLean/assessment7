...
describe("middleware/auth.js authUser BUG TEST", function() {
  // BUG TEST #1
  // test("should accept a request with a tempered token", async function(){
  //   // JWT w/ payload that has a different username
  //   const token = jwt.sign({ username: "tampered", admin: true}, "WRONG_SECRET_KEY");

  //   // Send request with tempered token
  //   const response = await request(app)
  //     .get("/users/u1")
  //     .send({ _token: token });

  //   // The tampered token request should be unauthorized, but it is not
  //   expect(response.statusCode).not.toBe(401);
  
  // })
  test("should not accept a request with an invalid SECERET_KEY", async function(){
    // JWT w/ payload that has a different username
    const token = jwt.sign({ username: "u1", admin: true}, "WRONG_SECRET_KEY");
